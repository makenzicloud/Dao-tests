...
rule noTotalSupplyChange(env e) {
    ...
    assert totalSupplyAfter > totalSupplyBefore => f.selector == sig:_mint(address,uint256).selector;
    assert totalSupplyAfter < totalSupplyBefore => f.selector == sig:_burn(address,uint256).selector;
