...
hook CALL(uint g, address addr, uint value, uint argsOffset, uint argsLength, uint retOffset, uint retLength) uint rc {
    ...
    if (g_sighash == sig:stakeAndLock().selector) {
        stakeAndLock@withrevert(e); // Concrete name
        g_reverted = lastReverted;
    }
    else if (g_sighash == sig:stakeAndLock().selector) {
        stakeAndLock@withrevert(e); // Concrete name
        g_reverted = lastReverted;
    }
    else if (g_sighash == sig:stakeAndLock().selector) {
        calldataarg args;
        stakeAndLock@withrevert(e, args); // Concrete name
        g_reverted = lastReverted;
    }
    else {
        // Fallback case
